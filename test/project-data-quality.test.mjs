...
// The test reads projectGroups directly from the extracted TS data module
// (lib/data/projects.ts), replacing the old approach of regex-slicing
// src/main.js. Pure-data extraction via eval of the object literal.
function loadProjectGroups() {
  const source = readFileSync(
    new URL("../lib/data/projects.ts", import.meta.url),
    "utf8",
  );
  const marker = "export const projectGroups: ";
  const start = source.indexOf("= ", source.indexOf(marker)) + 2;
  assert.notEqual(start, -1, "projectGroups declaration should exist");
  // The object literal ends at the first occurrence of "};" after the start.
  const closeIdx = source.indexOf("};", start);
  assert.notEqual(closeIdx, -1, "projectGroups literal should close with };");
  // Include up to and including the "}", drop the trailing ";".
  const objectSource = source.slice(start, closeIdx + 1).trim();
...
const auditReport = readFileSync(
  new URL("./first-page-project-audit.md", import.meta.url),
  "utf8",
);
