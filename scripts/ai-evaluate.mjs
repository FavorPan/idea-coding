...
// 输出: lib/generated/stars.ts + lib/generated/lastWeek.ts + lib/generated/metadata.ts
...
const OUTPUT_LAST_WEEK = path.join(ROOT, "lib", "generated", "lastWeek.ts");
...
async function evaluateCandidate(candidate) {
  ...
  try {
    ...
    return {
      ...
      weeklyStars: 0, // Actions 会更新 lastWeek.ts 后算出来
      trendingRank: 0, // 排序后填
      mvp,
      wow,
      useful,
...
function writeLastWeekTs(stars) {
  const counts = {};
  ...
  const header = `// AUTO-GENERATED by scripts/ai-evaluate.mjs — DO NOT EDIT BY HAND`;
  const body = `import type { LastWeekSnapshot } from "@/lib/generated/lastWeek";
export const lastWeekSnapshot: LastWeekSnapshot = ${JSON.stringify(snapshot, null, 2)};`;
  fs.writeFileSync(OUTPUT_LAST_WEEK, `${header}\n\n${body}\n`, "utf-8");
}
...
function rankProjects(stars) {
  ...
  // 重新按 trendingRank 排序
  ranked.sort((a, b) => b.weeklyStars - a.weeklyStars || b.totalStars - a.totalStars);
...
async function main() {
  ...
  console.log(`   📄 Written: lib/generated/lastWeek.ts (snapshot)`);
