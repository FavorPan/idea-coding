...
给 AI Coding 新手看的项目发现网站，外加一枚可以装进 Codex 的项目推荐 Skill：帮你找到值得搓的项目、匹配用得上的 Skill / 工具，并复制一段能直接交给 Codex、Cursor 或 Claude Code 的开工提示词。

**公开站点：** [https://ideacoding.favorhub.top](https://ideacoding.favorhub.top)

</div>

Idea Coding 帮你看清项目亮点，匹配用得上的 Skill / 工具，并生成一段能直接开工的提示词。
...
2. **评估** — `scripts/ai-evaluate.mjs` 读每个候选的 README 和 issue 区，交给 Agnes AI（`agnes-2.5-flash`）生成 `tagline` / `mvp` / `wow·useful·easy` 评分 / Skill 推荐，留 Top 30。
